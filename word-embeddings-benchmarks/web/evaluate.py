...
def evaluate_correlation(w_1, w_2, X):
    """
    Calculate Spearman correlation between cosine similarity of the model
    and human rated similarity of word pairs
    ...
    if isinstance(w_1, dict):
        w_1 = Embedding.from_dict(w_1)
    if isinstance(w_2, dict):
        w_2 = Embedding.from_dict(w_2)

    missing_words_1 = 0
    missing_words_2 = 0
    words_1 = w_1.vocabulary.word_id
    words_2 = w_2.vocabulary.word_id
    for query in X:
        for query_word in query:
            if query_word not in words_1:
                missing_words_1 += 1
            if query_word not in words_2:
                missing_words_2 += 1
    if missing_words_1 > 0:
        logger.warning("Missing {} words. Will replace them with mean vector".format(missing_words_1))
    if missing_words_2 > 0:
        logger.warning("Missing {} words. Will replace them with mean vector".format(missing_words_2))

    mean_vector_1 = np.mean(w_1.vectors, axis=0, keepdims=True)
    A_1 = np.vstack(w_1.get(word, mean_vector_1) for word in X[:, 0])
    B_1 = np.vstack(w_1.get(word, mean_vector_1) for word in X[:, 1])
    scores_1 = np.array([v1.dot(v2.T)/(np.linalg.norm(v1)*np.linalg.norm(v2)) for v1, v2 in zip(A_1, B_1)])

    mean_vector_2 = np.mean(w_2.vectors, axis=0, keepdims=True)
    A_2 = np.vstack(w_2.get(word, mean_vector_2) for word in X[:, 0])
    B_2 = np.vstack(w_2.get(word, mean_vector_2) for word in X[:, 1])
    scores_2 = np.array([v1.dot(v2.T)/(np.linalg.norm(v1)*np.linalg.norm(v2)) for v1, v2 in zip(A_2, B_2)])
    return scipy.stats.spearmanr(scores_1, scores_2).correlation
...
def evaluate_correlation_on_all(w_1, w_2):
    """
    Evaluate Embedding on all fast-running benchmarks
    ...
    # Calculate results on correlation
    logger.info("Calculating correlation benchmarks")
    correlation_tasks = {
        "MEN": fetch_MEN(),
        "WS353": fetch_WS353(),
        "WS353R": fetch_WS353(which="relatedness"),
    ...
    for name, data in iteritems(correlation_tasks):
        correlation_results[name] = evaluate_correlation(w_1, w_2, data.X)
        logger.info("Spearman correlation of scores on {} {}".format(name, correlation_results[name]))

    sim = pd.DataFrame([correlation_results])
    return sim
