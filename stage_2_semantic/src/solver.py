    ...
    def compute_train_loss(self, sess, summary_writer, summary_op, epoch, file_num, part,
        ...
        for step in tqdm(range(n_batches+1)):
            ...
            # batch_pos_feat, batch_neg_feats, batch_skip_feats, batch_labels \
                # = batch_pair_data(feats, feat_idx, skip_feat_idx, labels, spk2idx, idx2spk, masks, 
                                  # feat_indices, self.neg_sample_num)
            batch_neg_feats = batch_neg_feats.reshape((batch_size, self.neg_sample_num, self.feat_dim))
            ...
            if step % print_step == 0 and step != 0:
                ...
                print ('enc value:')
                print (enc_value[0][:10])
                print ('mask:')
                print (batch_masks[0])
                print ('context vector:')
                print (neighbor_encs_value[0][0][:10])
                print ('neg vector:')
                print (target_encs_value[0][0][:10])
