...
class AudioWord2Vec(object):
    ...
    def semantic_skip_encode(self, neighbor, reuse=False):
        with tf.variable_scope('semantic_skip_encode', reuse=reuse) as scope_skip_enc:
            ...
            W_enc_2 = tf.get_variable("skip_enc_w_2", [self.memory_dim*2, self.memory_dim])
            ...
            # neighbor = tf.contrib.layers.batch_norm(neighbor, is_training=self.is_training)
            enc = self.leaky_relu(tf.matmul(neighbor, W_enc_1) + b_enc_1)
            # enc = tf.matmul(neighbor, W_enc_1) + b_enc_1
            enc = tf.contrib.layers.batch_norm(enc, is_training=self.is_training)
            ...
            # enc = tf.matmul(enc, W_enc_3) + b_enc_3
            # enc = tf.contrib.layers.batch_norm(enc, is_training=self.is_training)
    ...
    def semantic_encode(self, feat, reuse=False):
        with tf.variable_scope('semantic_encode', reuse=reuse) as scope_enc:
            W_enc_1 = tf.get_variable("enc_w_1", [self.feat_dim, self.memory_dim*2])
            b_enc_1 = tf.get_variable("enc_b_1", shape=[self.memory_dim*2])
            W_enc_2 = tf.get_variable("enc_w_2", [self.memory_dim*2, self.memory_dim])
            ...
            # feat = tf.contrib.layers.batch_norm(feat, is_training=self.is_training)
    ...
    def build_model(self, is_training=True):
        ...
        with tf.variable_scope('generator') as scope_g:
            ...
            enc = self.semantic_encode(pos_feat)
            # enc = tf.clip_by_norm(enc, 1., axes=[-1])

            neighbor_encs = tf.reshape(neighbors, [-1, self.feat_dim])
            neighbor_encs = self.semantic_skip_encode(neighbor_encs)
            ...
            target_encs = self.semantic_skip_encode(targets, reuse=True)
            ...
            for i, neighbor in enumerate(neighbor_encs):
                ...
                # # target_encs = tf.clip_by_norm(target_encs, 1., axes=[-1])
            # dot_products = []
            for i, neg in enumerate(target_encs):
                product = tf.reduce_sum(neg*enc, -1)
