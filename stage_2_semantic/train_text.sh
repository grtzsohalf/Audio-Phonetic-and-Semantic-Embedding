...
python3 $path/src_text/train_text.py --init_lr=$init_lr --batch_size=$batch_size --feat_dim=$feat_dim \
  --hidden_dim=$dim --n_epochs=$n_epochs --gram_num=$gram_num --neg_sample_num=$neg_sample_num --min_count=$min_count \
  --sampling_factor=$sampling_factor $tf_log_dir $tf_model_dir $feat_dir/all_words $feat_dir/all_utters $embedding_dir/label2embedding
