...
exp_dir=$exp_dir/semantic_exps/text_sigmoid_exp
mkdir -p $exp_dir
...
embedding_dir=$feat_dir/embedding/text_sigmoid/${init_lr}_$dim
...
python3 $path/src_text/test_text.py --init_lr=$init_lr --batch_size=$batch_size --feat_dim=$feat_dim \
  --hidden_dim=$dim --gram_num=$gram_num --neg_sample_num=$neg_sample_num --min_count=$min_count $tf_log_dir $tf_model_dir \
  --sampling_factor=$sampling_factor $feat_dir/all_words $feat_dir/all_utters $embedding_dir/train_embeddings $embedding_dir/label2embedding
