...
def load_subsampled_data(example_file, label_file, utter_file, word_freq, total_count, 
    ...
    return count, feats, feat_idx, skip_feat_idx, labels, spk2idx, idx2spk, masks
...
def batch_pair_data(feats, feat_idx, skip_feat_idx, labels, spk2idx, idx2spk, masks, feat_indices, neg_num):
    ...
    for i in feat_indices:
        ...
        for ii in skip_feat_idx[i]:
            ...
            batch_skip_labels.append(labels[ii])
            batch_neg_feats.append([])
            batch_neg_labels.append([])
            for j in range(neg_num):
                ...
                while while_bool:
                    ...
                    if neg_label == labels[idx]:
                        while_bool = True
                        continue
                    for iii in skip_feat_idx[i]:
                        if neg_label == labels[iii]:
                            while_bool = True
                            break
                batch_neg_feats[-1].append(feats[neg_idx])
                batch_neg_labels[-1].append(labels[neg_idx])
