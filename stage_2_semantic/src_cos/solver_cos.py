    ...
    def compute_train_loss(self, sess, summary_writer, summary_op, epoch, file_num, part,
        ...
        for step in range(n_batches+1):
            ...
            batch_pos_feat, batch_neg_feats, batch_skip_feats, \
                batch_labels, batch_skip_labels, batch_neg_labels, batch_masks \
                = batch_pair_data(feats, feat_idx, skip_feat_idx, labels, spk2idx, idx2spk, masks, 
                                  feat_indices, self.neg_sample_num)
            batch_neg_feats = batch_neg_feats.reshape((-1, 2*self.gram_num, self.neg_sample_num, self.feat_dim))
            # batch_neg_feats = batch_neg_feats.reshape((-1, self.neg_sample_num, self.feat_dim))
            batch_skip_feats = batch_skip_feats.reshape((-1, 2*self.gram_num, self.feat_dim))
            # batch_skip_feats = batch_skip_feats.reshape((-1, self.feat_dim))
            batch_masks = batch_masks.reshape((-1, 2*self.gram_num))
            batch_neg_labels = batch_neg_labels.reshape((-1, 2*self.gram_num, self.neg_sample_num))
    ...
    def compute_test_loss(self, sess, summary_writer, summary_op, epoch, loss,
        ...
        for step in range(n_batches+1):
            ...
            # batch_pos_feat, batch_neg_feats, batch_skip_feats, batch_labels, batch_masks \
                # = batch_pair_data(feats, feat_idx, skip_feat_idx, labels, spk2idx, idx2spk, masks, 
                                  # feat_indices, self.neg_sample_num)
            batch_pos_feat, batch_neg_feats, batch_skip_feats, batch_labels, _, _, _ \
                = batch_pair_data(feats, feat_idx, skip_feat_idx, labels, spk2idx, idx2spk, masks, 
                                  feat_indices, self.neg_sample_num)
            batch_neg_feats = batch_neg_feats.reshape((-1, 2*self.gram_num, self.neg_sample_num, self.feat_dim))
            # batch_neg_feats = batch_neg_feats.reshape((-1, self.neg_sample_num, self.feat_dim))
        ...
        for epoch in range(self.n_epochs):
            ...
            for i, (example_file, label_file, utter_file) in enumerate(zip(example_files, label_files, utter_files)):
                self.n_feats, self.feats, self.feat_idx, self.skip_feat_idx, \
                    ...
                    = load_subsampled_data(os.path.join(self.example_dir, example_file), 
                                           os.path.join(self.label_dir, label_file),
                                           os.path.join(self.utter_dir, utter_file), 
                ...
                self.compute_train_loss(sess, summary_writer, summary_op_train, e, len(label_file), count,
                                        loss, first_products, enc, neighbor_encs, target_encs)
