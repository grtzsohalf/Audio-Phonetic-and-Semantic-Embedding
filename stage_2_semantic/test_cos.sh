...
python3 $path/src_cos/test_cos.py --init_lr=$init_lr --batch_size=$batch_size --feat_dim=$feat_dim \
  --hidden_dim=$dim --gram_num=$gram_num --neg_sample_num=$neg_sample_num --min_count=$min_count $tf_log_dir $tf_model_dir \
  --sampling_factor=$sampling_factor $feat_dir/all_examples_norm $feat_dir/all_words $feat_dir/all_utters $embedding_dir/train_embeddings
