...
if [ $# != 4 ] ; then 
  ...
  echo "e.g. train.sh 0.0001 256 0 20"
  exit 1
fi
...
exp_dir=$exp_dir/semantic_exps/cos_6_layer_sigmoid_exp
mkdir -p $exp_dir
...
embedding_dir=$feat_dir/embedding/cos_6_layer_sigmoid/${init_lr}_$dim
...
python3 $path/src_cos/train_cos.py --init_lr=$init_lr --batch_size=$batch_size --feat_dim=$feat_dim \
  --hidden_dim=$dim --n_epochs=$n_epochs --gram_num=$gram_num --neg_sample_num=$neg_sample_num --min_count=$min_count\
  --sampling_factor=$sampling_factor $tf_log_dir $tf_model_dir $feat_dir/all_examples_norm $feat_dir/all_words $feat_dir/all_utters #$train_num
