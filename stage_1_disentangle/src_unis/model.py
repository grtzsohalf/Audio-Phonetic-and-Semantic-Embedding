...
class Audio2Vec(object):
    ...
    def rec_loss(self, dec_out, feat):
        ...
        labels_trans = tf.stop_gradient(tf.reshape(feat, 
                shape=(self.seq_len*self.batch_size, self.feat_dim)))
        print(labels_trans.get_shape())
        # labels_trans = tf.reshape(labels_trans, shape=[-1])
        # dec_proj_outputs = tf.reshape(dec_out, shape=[-1])
        dec_proj_outputs = tf.transpose(dec_out)
        print(dec_proj_outputs.get_shape())
        ### compute RMSE error ###
        ### mask the zeroes while computing loss ###
        reconstruction_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(
            labels=labels_trans, logits=dec_proj_outputs,
