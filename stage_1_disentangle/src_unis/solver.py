...
class Solver(object):
    ...
    def train(self):
        ...
        ### Load data  ###
        feats_dir = os.path.join(self.feat_dir, 'pkls_unis', str(self.seq_len))
    ...
    def make_phonetic(self, all_AE_dir, phonetic_dir):
        ...
        ### ###
        feats_dir = os.path.join(self.feat_dir, 'pkls_unis', str(self.seq_len))
        for num in range(len(os.listdir(all_AE_dir))):
            all_AE_scp = os.path.join(all_AE_dir, 'all_AE_'+str(num)+'.scp')
